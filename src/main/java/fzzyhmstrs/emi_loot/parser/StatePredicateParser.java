    ...
    public static Text parseStatePredicate(StatePredicate predicate) {
        ...
        if (!list.isEmpty()) {
            ...
            for (Condition condition : list) {
                if (condition.valueMatcher() instanceof StatePredicate.RangedValueMatcher v) {
                    String key = condition.key();
                    String min = v.min().orElse(null);
                    String max = v.max().orElse(null);
                    list2.add(LText.translatable("emi_loot.state_predicate.state_between", key, min, max));
                } else if (condition.valueMatcher() instanceof StatePredicate.ExactValueMatcher v) {
                    String key = condition.key();
